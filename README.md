...
一个面向电商卖家的 AI 商品图片生成工具，支持 C 端注册登录、B 端为用户配置模型 API Key、商品详情页套图、单图生成、二次编辑、本地素材管理，以及配套 B 端用户和生成日志管理。
...
- C 端用户必须注册 / 登录后才能生成图片；API Key 由 B 端为每个用户配置。
- C 端只显示脱敏后的 API Key 状态，接口地址和模型由 B 端默认配置控制。
- 工具不提供套餐、充值或配额扣减。
- 后端保存用户信息、会话、模型配置、生成调用日志和用量统计。
- B 端登录页独立于管理台；登录后可以查看所有注册用户、配置/清空用户 API Key、禁用/启用用户、查看图片反馈、查看所有生成记录的入参/出参日志、查看调用次数和 token 数，并配置默认模型与提示词。
...
- 查看所有用户姓名、邮箱、注册时间、最近登录时间和 API Key 脱敏配置状态
- 为每个注册用户配置或清空模型 API Key
...
- 用户 API Key 只保存在后端 SQLite，不通过 C 端接口返回明文
