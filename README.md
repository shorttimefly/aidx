...
一个面向电商卖家的 AI 商品图片生成工具，支持 C 端注册登录、B 端配置图片/视频模型供应商、按用户授权可用模型、商品详情页套图、单图生成、二次编辑、本地素材管理，以及配套 B 端用户和生成日志管理。
...
- C 端用户必须注册 / 登录后才能生成图片；供应商 URL、Token 和模型名由 B 端在模型供应商列表统一配置，再给具体用户选择可用图片/视频模型。
- 默认注册用户都是普通用户，只能使用 C 端；只有内置 B 端管理员或被后台授予管理员角色的注册用户可以登录 B 端。
- C 端只显示脱敏后的模型状态；图片模型选择由服务端根据 B 端授权、默认模型和优先级自动处理，不提供用户手动切换模型入口。
- 工具不提供套餐、充值或配额扣减。
- 后端保存用户信息、会话、模型配置、生成调用日志和用量统计。
- B 端登录页独立于管理台；登录后可以查看所有注册用户、设置用户角色、给用户选择可用图片/视频模型、禁用/启用用户、配置模型供应商、查看图片反馈、查看所有生成记录的入参/出参日志、查看调用次数和 token 数，并分别配置图片默认模型、视频默认模型与提示词。
...
B 端还可以在模型供应商列表中新增独立模型供应商，并通过弹窗填写 URL、Token、模型名、模型类型和优先级。模型类型可选图片模型或视频模型；图片和视频分别有一个默认模型。当前图片生成内置适配：
...
当用户被授权了一个或多个图片供应商模型时，`POST /api/generate` 会按模型优先级从小到大尝试；上游 429/5xx/网络/超时/未返回图片等失败会切到下一个模型。用户没有新授权时，优先使用 B 端列表里设定的默认图片模型；没有默认图片模型时，完整回退到旧的单用户图片 Key、地址和模型配置。

## B 端能力

- 查看所有用户名、邮箱状态、注册时间、最近登录时间和图片/视频模型授权状态
- 为每个注册用户选择可用图片模型和可用视频模型；未指定时使用对应默认模型
- 配置模型供应商列表：新增/修改弹窗填写供应商名称、类型、URL、Token、模型名、模型类型和优先级，并可在列表中分别设为默认图片模型或默认视频模型
- 在用户配置弹窗中为每个用户授权多个可用图片模型或视频模型
- 查看每个用户调用次数、生成图片数、输入 token、输出 token、总 token
- 禁用或启用用户；禁用后用户会话失效，前端生成前会被拦截
- 查看最近生成记录，包括用户、模型、状态、耗时、token 和脱敏后的请求/响应 JSON
- 配置提示词，并在模型供应商列表中设置 C 端默认模型
...
- 供应商 Token 只保存在后端 SQLite，不通过 C 端接口返回明文
- 新模型供应商体系使用 `model_providers`、`provider_models`、`user_model_access` 三张表；`provider_models.model_kind` 区分图片/视频模型；旧 `user_settings.api_key/endpoint/model` 仍作为无新授权和无默认图片模型时的兼容兜底
