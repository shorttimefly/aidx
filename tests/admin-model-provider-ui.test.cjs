...
assert.ok(html.includes("文本理解模型"));
...
assert.ok(admin.includes("function isPromptFactoryCompatibleModelConfig"));
assert.ok(admin.includes("function defaultModelNameForProvider"));
assert.ok(admin.includes('return "gpt-5.5"'));
assert.ok(admin.includes('showToast("AOKAPI / Gemini 需要填写 Gemini generateContent URL 和 Gemini 图片模型名", true)'));
...
assert.ok(admin.includes("OpenAI Compatible"));
