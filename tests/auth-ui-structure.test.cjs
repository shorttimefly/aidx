...
assert.ok(app.includes("videoModelSettings"));
assert.ok(app.includes("renderAccountKeyStatus"));
assert.ok(app.includes("function accountModelName"));
assert.ok(app.includes("function accountConfiguredText"));
assert.ok(app.includes("state.auth.modelSettings.model"));
assert.ok(app.includes("state.auth.videoModelSettings.model"));
assert.ok(app.includes('model: settings.videoModel || ""'));
assert.ok(app.includes('return accountModelName(modelName) || "未配置";'));
assert.ok(!app.includes('return String(modelName || "").trim() || "已配置";'));
assert.ok(html.includes("./app.js?v=20260612-model-status"));
